...
role_list = ["Material", "Method", "Metric", "Task"]
...
class NERTransformer(BaseTransformer):
    ...
    def __init__(self, hparams):
        ...
        self.device = torch.device(
            "cuda" if torch.cuda.is_available() and self.hparams.n_gpu else "cpu")
        ...
        self.SEP_template = self.tokenizer.convert_tokens_to_ids(["[unused0]"])[
            0]
    ...
    def forward(self, **inputs):
        ...
        src_sequence_output = torch.transpose(
            src_sequence_output, 1, 2)  # hidden * doc_length
        # tgt_length * hidden
        tgt_sequence_output = sequence_output[:, args.max_seq_length_src:, :]
        # tgt_length * doc_length
        logits = torch.bmm(tgt_sequence_output, src_sequence_output)
    ...
    def training_step(self, batch, batch_num):
        ...
        inputs = {"input_ids": batch[0], "attention_mask": batch[1],
                  "token_type_ids": batch[2], "position_ids": batch[3], "labels": batch[4]}
        outputs = self(**inputs)
        loss = outputs[0]
        ...
        for mode in ["train", "dev", "test"]:
            ...
            if not os.path.exists(cached_features_file):
                ...
                examples = read_examples_from_file(
                    args.data_dir, mode, self.tokenizer, debug=args.debug)
    ...
    def test_step(self, batch, batch_nb):
        ...
        tgt_input_ids, init_tgt_input_ids = torch.tensor([[self.CLS]]).to(
            self.device), torch.tensor([[self.CLS]]).to(self.device)
        tgt_position_ids, init_tgt_position_ids = torch.tensor(
            [[0]]).to(self.device), torch.tensor([[0]]).to(self.device)
        ...
        while i <= max_seq_length_src + max_seq_length_tgt - 1:
            ...
            position_ids = torch.cat(
                (src_position_ids, tgt_position_ids), dim=1)
            ...
            for j in range(top_2_indices.size(0)):
                prob_gap = (top_2_probs[j][0]/top_2_probs[j]
                            [1]).detach().cpu().tolist()
            ...
            out_input_id = torch.index_select(
                src_input_ids, 1, out_position_id)
            out_position_id = out_position_id.unsqueeze(dim=0)  # add batch dim
            tgt_input_ids = torch.cat(
                (init_tgt_input_ids, out_input_id), dim=1)
        ...
        for b in range(bs):  # bs == 1
            ...
            for idx, token_id in enumerate(out_input_id_list):
                if token_id == self.CLS:
                    break
            pred_seq.append(self.tokenizer.convert_ids_to_tokens(
                out_input_id_list[:idx+1]))
    ...
    def test_epoch_end(self, outputs):
        ...
        doctexts_tokens, golds = read_golds_from_test_file(
            args.data_dir, self.tokenizer, debug=args.debug)
        ...
        for x in outputs:
            ...
            for docid, p_seq in zip(docids, pred_seq):
                if docid not in preds_log:
                    ...
                    preds_log[docid]["doctext"] = " ".join(
                        doctexts_tokens[docid])
        ...
        for key in results:
            ...
            print("P: {:.2f}%,  R: {:.2f}%, F1: {:.2f}%".format(
                results[key]["p"] * 100, results[key]["r"] * 100, results[key]["f1"] * 100))  # phi_strict
