...
def score(mapping, pred, gold):
    ...
    for key in all_keys:
        ex_result[key] = {"p_num": 0, "p_den": 0,
                          "r_num": 0, "r_den": 0, "p": 0, "r": 0, "f1": 0}
    ...
    for key in all_keys:
        ex_result[key]["p"] = 0 if ex_result[key]["p_num"] == 0 else ex_result[key]["p_num"] / \
            float(ex_result[key]["p_den"])
        ex_result[key]["r"] = 0 if ex_result[key]["r_num"] == 0 else ex_result[key]["r_num"] / \
            float(ex_result[key]["r_den"])
        ex_result[key]["f1"] = f1(ex_result[key]["p_num"], ex_result[key]
                                  ["p_den"], ex_result[key]["r_num"], ex_result[key]["r_den"])
...
def eval_tf(preds, golds, docids=[]):
    ...
    for docid in docids:
        ...
        V = V + [-1]
        init_maps = [dict(zip(K, p))
                     for p in itertools.product(V, repeat=len(K))]
...
if __name__ == "__main__":
    ...
    if args.event_n == "1,2,3,4":
        ...
        for key in all_keys:
            ...
            str_print += [results[key]["p"] * 100, results[key]
                          ["r"] * 100, results[key]["f1"] * 100]
